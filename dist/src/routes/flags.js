...
flagsRoute.get("/", async (c) => {
	...
	let envFilter;
	if (environment) {
		if (!Object.values(FeatureEnvironment).includes(environment)) {
			return c.json(
...
const fetchFlagWithRelations = (key) => {
...
const upsertUserTargets = async (flagId, userTargets) => {
...
const replaceUserTargets = async (flagId, userTargets) => {
	const envs = await prisma.featureFlagEnvironment.findMany({
		where: { flagId },
	});
	const envMap = new Map(envs.map((env) => [env.environment, env.id]));
